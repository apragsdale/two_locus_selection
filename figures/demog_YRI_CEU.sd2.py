...
def get_sd2(
    ...
    cached_fname = f"data/time_series/cache/{name}-n_{n}-rho_{rho}-gamma_{gamma}-h_{h}-eps_{eps}-dom_simple-spacing_{spacing}.sd2.bp"
...
def get_sd2_cond(
    ...
    n = 50
    cached_fname = f"data/time_series/cache/{name}-n_{n}-rho_{rho}-gamma_{gamma}-h_{h}-eps_{eps}-dom_simple-spacing_{spacing}.sd2.nAmin_{nAmin}.nAmax_{nAmax}.nBmin_{nBmin}.nBmax_{nBmax}bp"
    if os.path.exists(cached_fname):
        ts, v = pickle.load(open(cached_fname, "rb"))
        return ts, v
    data = pickle.load(
...
plt.savefig("demog_YRI_CEU.sd2.pdf")
