...
def get_sd1(
    nus, Ts, n, Ne, n_large=None, rho=0.0, gamma=0, eps=0.0, h=0.5, spacing=5, name=None
):
    # see if we cached the t, sd1 vector data, so we don't need to recompute
    cached_fname = f"data/time_series/cache/{name}-n_{n}-rho_{rho}-gamma_{gamma}-h_{h}-eps_{eps}-dom_simple-spacing_{spacing}.sd1.bp"
...
def get_sd1_cond(
    ...
    n = 50
    cached_fname = f"data/time_series/cache/{name}-n_{n}-rho_{rho}-gamma_{gamma}-h_{h}-eps_{eps}-dom_simple-spacing_{spacing}.sd1.nAmin_{nAmin}.nAmax_{nAmax}.nBmin_{nBmin}.nBmax_{nBmax}bp"
    if os.path.exists(cached_fname):
        ts, v = pickle.load(open(cached_fname, "rb"))
        return ts, v
    data = pickle.load(
...
# gamma = -1
t, v = get_sd1(nu_YRI, Ts, 50, Ne, gamma=-1.0, name="YRI")
axes[1].plot(1e6 - gen_time * t, v, "-", lw=lw, color=c1[1])
t, v = get_sd1(nu_CEU, Ts, 50, Ne, gamma=-1.0, name="CEU")
axes[1].plot(1e6 - gen_time * t, v, "--", lw=lw, color=c2[1])
axes[1].text(2e5, v[0] + 0.02, rf"$\gamma=-1$", va="center", ha="center", fontsize=7)

# gamma = -10
t, v = get_sd1(nu_YRI, Ts, 50, Ne, n_large=70, gamma=-10.0, name="YRI")
axes[1].plot(1e6 - gen_time * t, v, "-", lw=lw, color=c1[1])
t, v = get_sd1(nu_CEU, Ts, 50, Ne, n_large=70, gamma=-10.0, name="CEU")
axes[1].plot(1e6 - gen_time * t, v, "--", lw=lw, color=c2[1])
...
axes[1].set_title(r"Interference ($\epsilon=0$)")

## synergistic epistasis
print("plotting positive epistasis panel")
# gamma = -1
t, v = get_sd1(nu_YRI, Ts, 50, Ne, gamma=-1.0, eps=0.5, name="YRI")
axes[2].plot(1e6 - gen_time * t, v, "-", lw=lw, color=c1[2])
t, v = get_sd1(nu_CEU, Ts, 50, Ne, gamma=-1.0, eps=0.5, name="CEU")
axes[2].plot(1e6 - gen_time * t, v, "--", lw=lw, color=c2[2])
axes[2].text(2e5, v[0] + 0.06, rf"$\gamma=-1$", va="center", ha="center", fontsize=7)

# gamma = -10
t, v = get_sd1(nu_YRI, Ts, 50, Ne, n_large=70, gamma=-10.0, eps=0.5, name="YRI")
axes[2].plot(1e6 - gen_time * t, v, "-", lw=lw, color=c1[2])
t, v = get_sd1(nu_CEU, Ts, 50, Ne, n_large=70, gamma=-10.0, eps=0.5, name="CEU")
axes[2].plot(1e6 - gen_time * t, v, "--", lw=lw, color=c2[2])
...
## each with gamma = -1
## eps = 0
t, v = get_sd1_cond(Ne, gamma=-1.0, eps=0.0, name="YRI", nAmax=thr, nBmax=thr)
axes[4].plot(1e6 - gen_time * t, v, "-", lw=lw, color=c1[1])
t, v = get_sd1_cond(Ne, gamma=-1.0, eps=0.0, name="CEU", nAmax=thr, nBmax=thr)
axes[4].plot(1e6 - gen_time * t, v, "--", lw=lw, color=c2[1])
#axes[4].text(2500, v[0], rf"$\epsilon=0$", va="center", ha="center", fontsize=7)
# eps = 0.5
t, v = get_sd1_cond(Ne, gamma=-1.0, eps=0.5, name="YRI", nAmax=thr, nBmax=thr)
axes[4].plot(1e6 - gen_time * t, v, "-", lw=lw, color=c1[2])
t, v = get_sd1_cond(Ne, gamma=-1.0, eps=0.5, name="CEU", nAmax=thr, nBmax=thr)
...
# each with gamma = -1
# eps = 0
t, v = get_sd1_cond(Ne, gamma=-1.0, eps=0.0, name="YRI", nAmin=thr + 1, nBmin=thr + 1)
axes[5].plot(1e6 - gen_time * t, v, "-", lw=lw, color=c1[1])
t, v = get_sd1_cond(Ne, gamma=-1.0, eps=0.0, name="CEU", nAmin=thr + 1, nBmin=thr + 1)
axes[5].plot(1e6 - gen_time * t, v, "--", lw=lw, color=c2[1])
axes[5].text(2e5, v[0] - 0.2, rf"$\epsilon=0$", va="center", ha="center", fontsize=7)
# eps = 0.5
t, v = get_sd1_cond(Ne, gamma=-1.0, eps=0.5, name="YRI", nAmin=thr + 1, nBmin=thr + 1)
axes[5].plot(1e6 - gen_time * t, v, "-", lw=lw, color=c1[2])
t, v = get_sd1_cond(Ne, gamma=-1.0, eps=0.5, name="CEU", nAmin=thr + 1, nBmin=thr + 1)
