...
if __name__ == "__main__":
    ...
    # compute for rare mutations
    num_pairs_by_gene_within_rare, LD_by_gene_within_rare = compute_LD_by_gene_list(
        genotype_counts, nAmax=rare_n, nBmax=rare_n
    )

    # compute for uncommon mutations
    (
        num_pairs_by_gene_within_uncommon,
        LD_by_gene_within_uncommon,
    ...
    # compute for common mutations
    num_pairs_by_gene_within_common, LD_by_gene_within_common = compute_LD_by_gene_list(
    ...
    # compute for rare
    num_pairs_by_gene_outside_rare, LD_by_gene_outside_rare = compute_LD_by_gene_list(
        genotype_counts_outside, nAmax=rare_n, nBmax=rare_n
    )

    # compute for uncommon
    (
        num_pairs_by_gene_outside_uncommon,
        LD_by_gene_outside_uncommon,
    ) = compute_LD_by_gene_list(
        genotype_counts_outside,
        nAmin=rare_n + 1,
        nAmax=moderate_n,
    ...
    # compute for common
    (
        num_pairs_by_gene_outside_common,
        LD_by_gene_outside_common,
    ) = compute_LD_by_gene_list(
        genotype_counts_outside, nAmin=moderate_n + 1, nBmin=moderate_n + 1
    )
    ...
    # pairs (rare) within domains by distances
    (
        num_pairs_by_gene_within_bins_rare,
        LD_by_gene_within_bins_rare,
    ) = compute_LD_by_gene_list_bins(
        genotype_counts,
        distances_within,
        bin_edges,
    ...
    # pairs (uncommon) within domains by distances
    (
        num_pairs_by_gene_within_bins_uncommon,
        LD_by_gene_within_bins_uncommon,
    ) = compute_LD_by_gene_list_bins(
        genotype_counts,
        distances_within,
        bin_edges,
    ...
    # pairs (common) within domains by distances
    (
        num_pairs_by_gene_within_bins_common,
        LD_by_gene_within_bins_common,
    ) = compute_LD_by_gene_list_bins(
        genotype_counts,
        distances_within,
        bin_edges,
    ...
    # pairs (rare) within domains by distances
    (
        num_pairs_by_gene_outside_bins_rare,
        LD_by_gene_outside_bins_rare,
    ) = compute_LD_by_gene_list_bins(
        genotype_counts_outside,
        distances_outside,
        bin_edges,
    ...
    # pairs (uncommon) within domains by distances
    (
        num_pairs_by_gene_outside_bins_uncommon,
        LD_by_gene_outside_bins_uncommon,
    ) = compute_LD_by_gene_list_bins(
        genotype_counts_outside,
        distances_outside,
        bin_edges,
    ...
    # pairs (common) within domains by distances
    (
        num_pairs_by_gene_outside_bins_common,
        LD_by_gene_outside_bins_common,
    ) = compute_LD_by_gene_list_bins(
        genotype_counts_outside,
        distances_outside,
        bin_edges,
    ...
    data["within"] = {
        ...
        "rare": get_data(LD_by_gene_within_rare),
        "uncommon": get_data(LD_by_gene_within_uncommon),
        "common": get_data(LD_by_gene_within_common),
    }
    data["outside"] = {}
    data["outside"]["all_pairs"] = {
        "all": get_data(LD_by_gene_outside),
        "rare": get_data(LD_by_gene_outside_rare),
        "uncommon": get_data(LD_by_gene_outside_uncommon),
        "common": get_data(LD_by_gene_outside_common),
    }
    data["outside"]["matching"] = {
        "all": get_matching_stats(
            LD_by_gene_outside_bins, num_pairs_by_gene_within_bins, bin_edges
        ),
        "rare": get_matching_stats(
            LD_by_gene_outside_bins_rare,
            num_pairs_by_gene_within_bins_rare,
            bin_edges,
        ),
        "uncommon": get_matching_stats(
            LD_by_gene_outside_bins_uncommon,
            num_pairs_by_gene_within_bins_uncommon,
            bin_edges,
        ),
        "common": get_matching_stats(
            LD_by_gene_outside_bins_common,
            num_pairs_by_gene_within_bins_common,
            bin_edges,
        ),
    ...
    pickle.dump(data, open(f"parsed_data_v2/{POP}.unphased.domains.0.02_to_0.1.bp", "wb+"))

    # get standard errors
    SEs = {}
    SEs["within"] = {
        "all": bootstrap_genes(LD_by_gene_within),
        "rare": bootstrap_genes(LD_by_gene_within_rare),
        "uncommon": bootstrap_genes(LD_by_gene_within_uncommon),
        "common": bootstrap_genes(LD_by_gene_within_common),
    }
    SEs["outside"] = {}
    SEs["outside"]["all_pairs"] = {
        "all": bootstrap_genes(LD_by_gene_outside),
        "rare": bootstrap_genes(LD_by_gene_outside_rare),
        "uncommon": bootstrap_genes(LD_by_gene_outside_uncommon),
        "common": bootstrap_genes(LD_by_gene_outside_common),
    }
    SEs["outside"]["matching"] = {
        "all": bootstrap_matching(
            LD_by_gene_outside_bins, num_pairs_by_gene_within_bins, bin_edges
        ),
        "rare": bootstrap_matching(
            LD_by_gene_outside_bins_rare,
            num_pairs_by_gene_within_bins_rare,
            bin_edges,
        ),
        "uncommon": bootstrap_matching(
            LD_by_gene_outside_bins_uncommon,
            num_pairs_by_gene_within_bins_uncommon,
            bin_edges,
        ),
        "common": bootstrap_matching(
            LD_by_gene_outside_bins_common,
            num_pairs_by_gene_within_bins_common,
            bin_edges,
        ),
    ...
    pickle.dump(SEs, open(f"parsed_data_v2/{POP}.unphased.domains.0.02_to_0.1.SEs.bp", "wb+"))
