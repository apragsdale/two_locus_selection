...
def get_two_locus_genotype_counts_between_domains(genes, G, pos, dom):
    """
    For each gene, return genotype counts for each configuration seen.
    """
    genes_set = set(genes)
    genes_genotype_counts = defaultdict(list)
    genes_distances = defaultdict(list)
    for gene in genes_set:
        if gene not in dom.keys():
            continue
        ...
        for dom_interval in dom[gene]:
            ...
            if len(pos_dom) == 0:
                continue
            G_doms.append(G_dom)
            pos_doms.append(pos_dom)

        if len(pos_doms) < 2:
            continue
        num_regions = len(pos_doms)
        for i in range(num_regions - 1):
            for j in range(i + 1, num_regions):
                ...
                L2, n2 = G2.shape
                assert n1 == n2
                G_dict1, any_missing = moments.LD.Parsing._sparsify_genotype_matrix(G1)
                G_dict2, any_missing = moments.LD.Parsing._sparsify_genotype_matrix(G2)
                Counts = moments.LD.Parsing.spt.count_genotypes_between_sparse(
                    G_dict1, G_dict2, n1
                )
                # each row of Counts is
                # [nAABB, nAABb, nAAbb, nAaBB, nAaBb, nAabb, naaBB, naaBb, naabb]
                genes_genotype_counts[gene].append(Counts)
                genes_distances[gene].append(np.array(ds))
    return genes_genotype_counts, genes_distances
...
if __name__ == "__main__":
    ...
    ## between domains
    genotype_counts_between = {}
    distances_between = {}
    for chrom in chroms:
        genotype_counts_between[chrom] = {}
        distances_between[chrom] = {}
        #print("processing chromosome", chrom)
        positions, annotations, genes, G = load_genotype_matrix(chrom, POP)
        # get stats between domains
        for annot in annots:
            pos_sub, genes_sub, G_sub = subset_annotation(
                positions, annotations, genes, G, annot=annot
            )
            cs, ps = get_two_locus_genotype_counts_between_domains(
                genes_sub, G_sub, pos_sub, domains[chrom]
            )
            genotype_counts_between[chrom][annot] = cs
            distances_between[chrom][annot] = ps

    # compute for all
    num_pairs_by_gene_between, LD_by_gene_between = compute_LD_by_gene_list(
        genotype_counts_between
    )

    # compute for n <= 2
    num_pairs_by_gene_between_leq_2, LD_by_gene_between_leq_2 = compute_LD_by_gene_list(
        genotype_counts_between, nAmax=2, nBmax=2
    )

    # compute for 3 <= n <= 8
    (
        num_pairs_by_gene_between_3_to_8,
        LD_by_gene_between_3_to_8,
    ) = compute_LD_by_gene_list(
        genotype_counts_between, nAmin=3, nAmax=8, nBmin=3, nBmax=8
    )

    # compute for n >= 9
    num_pairs_by_gene_between_geq_9, LD_by_gene_between_geq_9 = compute_LD_by_gene_list(
        genotype_counts_between, nAmin=9, nBmin=9
    )
    ...
    ## outside domains, matched to between domain distances
    ...
    # all pairs between domains by distance
    (
        num_pairs_by_gene_between_bins,
        LD_by_gene_between_bins,
    ...
    # pairs n <= 2 between domains by distances
    (
        num_pairs_by_gene_between_bins_leq_2,
        LD_by_gene_between_bins_leq_2,
    ) = compute_LD_by_gene_list_bins(
        genotype_counts_between,
        distances_between,
        bin_edges,
    ...
    # pairs 3 <= n <= 8 between domains by distances
    (
        num_pairs_by_gene_between_bins_3_to_8,
        LD_by_gene_between_bins_3_to_8,
    ) = compute_LD_by_gene_list_bins(
        genotype_counts_between,
        distances_between,
        bin_edges,
    ...
    # pairs n >= 9 between domains by distances
    (
        num_pairs_by_gene_between_bins_geq_9,
        LD_by_gene_between_bins_geq_9,
    ) = compute_LD_by_gene_list_bins(
        genotype_counts_between,
        distances_between,
        bin_edges,
    ...
    # pairs n <= 2 outside domains by distances
    (
    ...
    # pairs 3 <= n <= 8 outside domains by distances
    (
    ...
    # pairs n >= 9 outside domains by distances
    (
    ...
    ## get all data (between and outside) and save
    data = {}
    data["between"] = {
        "all": get_data(LD_by_gene_between),
        "leq2": get_data(LD_by_gene_between_leq_2),
        "3to8": get_data(LD_by_gene_between_3_to_8),
        "geq9": get_data(LD_by_gene_between_geq_9),
    }
    data["outside"] = {}
    ...
    data["outside"]["matching"] = {
        "all": get_matching_stats(
            LD_by_gene_outside_bins, num_pairs_by_gene_between_bins, bin_edges
        ),
    ...
    data["bins"] = {
        "between": get_data_binned(LD_by_gene_between_bins),
        "outside": get_data_binned(LD_by_gene_outside_bins),
    }
    data["bin_edges"] = bin_edges

    pickle.dump(data, open(f"parsed_data_v2/{POP}.unphased.between_domains.bp", "wb+"))

    # get standard errors
    SEs = {}
    SEs["between"] = {
        "all": bootstrap_genes(LD_by_gene_between, annots=annots),
        "leq2": bootstrap_genes(LD_by_gene_between_leq_2, annots=annots),
        "3to8": bootstrap_genes(LD_by_gene_between_3_to_8, annots=annots),
        "geq9": bootstrap_genes(LD_by_gene_between_geq_9, annots=annots),
    }
    SEs["outside"] = {}
    SEs["outside"]["all_pairs"] = {
        "all": bootstrap_genes(LD_by_gene_outside, annots=annots),
        "leq2": bootstrap_genes(LD_by_gene_outside_leq_2, annots=annots),
        "3to8": bootstrap_genes(LD_by_gene_outside_3_to_8, annots=annots),
        "geq9": bootstrap_genes(LD_by_gene_outside_geq_9, annots=annots),
    }
    ...
    pickle.dump(SEs, open(f"parsed_data_v2/{POP}.unphased.between_domains.SEs.bp", "wb+"))
