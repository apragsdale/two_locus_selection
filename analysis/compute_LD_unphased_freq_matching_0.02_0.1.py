...
def compute_LD_by_gene(genotype_counts, nAmin=None, nAmax=None, nBmin=None, nBmax=None, annots=["synonymous", "missense", "loss_of_function"]):
    """
...
def get_data(LD_data, annots=["synonymous", "missense", "loss_of_function"]):
    data_out = {}
    for annot in annots:
        data_out[annot] = {}
...
def get_data_binned(LD_data, annots=["synonymous", "missense", "loss_of_function"]):
    data_out = {}
    for annot in annots:
...
if __name__ == "__main__":
    ...
    # compute for rare
    num_pairs_by_gene_rare, LD_by_gene_rare = compute_LD_by_gene(
        genotype_counts, nAmax=rare_n, nBmax=rare_n
    )

    # compute for uncommon
    num_pairs_by_gene_uncommon, LD_by_gene_uncommon = compute_LD_by_gene(
        genotype_counts, nAmin=rare_n+1, nBmin=rare_n+1, nAmax=moderate_n, nBmax=moderate_n
    )

    # compute for common
    num_pairs_by_gene_common, LD_by_gene_common = compute_LD_by_gene(
    ...
    data["all_sites"] = {
        ...
        "rare": get_data(LD_by_gene_rare),
        "uncommon": get_data(LD_by_gene_uncommon),
        "common": get_data(LD_by_gene_common),
    }
    data["bins"] = {
    ...
    pickle.dump(data, open(f"parsed_data_v2/{POP}.unphased.all.0.02_to_0.1.bp", "wb+"))

    SEs = {}
    SEs["all_sites"] = {
        "all": bootstrap_genes(LD_by_gene),
        "rare": bootstrap_genes(LD_by_gene_rare),
        "uncommon": bootstrap_genes(LD_by_gene_uncommon),
        "common": bootstrap_genes(LD_by_gene_common),
    }
    SEs["bins"] = {"all_sites": bootstrap_genes_bins(LD_by_gene_bins)}

    pickle.dump(SEs, open(f"parsed_data_v2/{POP}.unphased.all.SEs.0.02_to_0.1.bp", "wb+"))
