...
if __name__ == "__main__":
    ...
    ## within domains
    D_in_domains = {annot: defaultdict(float) for annot in annots}
    pi2_in_domains = {annot: defaultdict(float) for annot in annots}
    all_genes = set()
    kept_genes = set()
    for chrom in chroms:
        ...
        gene_set = set(genes)
        all_genes = all_genes.union(gene_set)
        ...
        for annot in annots:
            ...
            for k, v in cs.items():
                ...
                pi2 = 0
                for gs in v:
                    D += moments.LD.Parsing.gcs.compute_D(gs).sum()
                    pi2 += moments.LD.Parsing.gcs.compute_pi2(gs).sum()
                D_in_domains[annot][k] = D
                pi2_in_domains[annot][k] = pi2
                kept_genes = kept_genes.union({k})
    ...
    for i in range(n_reps):
        bs_genes = np.random.choice(list(kept_genes), len(kept_genes))
        D_syn = sum([D_in_domains["synonymous"][g] for g in bs_genes])
        pi2_syn = sum([pi2_in_domains["synonymous"][g] for g in bs_genes])
        D_mis = sum([D_in_domains["missense"][g] for g in bs_genes])
        pi2_mis = sum([pi2_in_domains["missense"][g] for g in bs_genes])
        sd1s.append([D_syn / pi2_syn, D_mis / pi2_mis])

    x = np.array(sd1s)
    num_neg = sum(x[:, 1] - x[:, 0] < 0)
    p_neg = num_neg / len(x)
