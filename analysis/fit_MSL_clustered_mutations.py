...
data = pickle.load(open("parsed_data/MSL.unphased.decay.all.bp", "rb"))

num_bins = 12

bins = data["bins"][:num_bins]
bin_mids = np.mean(bins, axis=1)

sd1 = np.array([v[3] / v[2] for v in data["ld"]["synonymous"].values()][:num_bins])
sd2 = np.array([v[0] / v[2] for v in data["ld"]["synonymous"].values()][:num_bins])
v1 = np.array(
    [v ** 2 for v in data["standard_errors"]["synonymous"]["sigmad1"].values()][
        :num_bins
    ]
)
v2 = np.array(
    [v ** 2 for v in data["standard_errors"]["synonymous"]["sigmad2"].values()][
        :num_bins
    ]
)
...
fout = open("mnm_optimization.sd1.txt", "w+")
