...
if __name__ == "__main__":
    ...
    mutations_by_domain = {annot: {} for annot in annots}
            ...
            for chrom in chroms:
                ...
                for annot in annots:
                    ...
                    for gene in gene_set:
                        mutations_by_domain[annot].setdefault(gene, {})
                        to_keep = genes_sub == gene
                        pos_gene = pos_sub.compress(to_keep)
                        for ii, dom in enumerate(domains[chrom][gene]):
                            mutations_by_domain[annot][gene].setdefault(ii, set())
                            to_keep = np.logical_and(
                                pos_gene >= dom[0], pos_gene <= dom[1]
                            )
                            pos_dom = pos_gene.compress(to_keep)
                            for pos in pos_dom:
                                mutations_by_domain[annot][gene][ii].add(pos)

    all_genes = set.union(
        set(mutations_by_domain["synonymous"].keys()),
        set(mutations_by_domain["missense"].keys()),
    )

    count_pairs = []
    # [num_syn, num_mis]
    for gene in all_genes:
        for i in range(len(domains[chrom][gene])):
            if gene in mutations_by_domain["synonymous"]:
                num_syn = len(mutations_by_domain["synonymous"][gene][i])
            else:
                num_syn = 0
            if gene in mutations_by_domain["missense"]:
                num_mis = len(mutations_by_domain["missense"][gene][i])
            else:
                num_mis = 0
            count_pairs.append([num_syn, num_mis])

    count_pairs = np.array(count_pairs)
    ...
    maxx, maxy = np.max(count_pairs, axis=0)
    ax.set_xlim(-1, 21)
    ax.set_ylim(-1, 26)
    ...
    ax.legend()
    ax.set_title("Numbers of mutations within the same domain")
    fig.tight_layout()
    plt.savefig("../figures/mutations_in_domains.pdf")
