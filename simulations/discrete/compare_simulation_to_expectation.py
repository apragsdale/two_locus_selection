...
def plot_grid(model, data, args):
    ...
    fig.suptitle(rf"$N={args.population_size}, n={args.sample_size}, s=-{args.selection_coefficient}, \epsilon={args.epistasis_coefficient}, r={args.recombination_rate}$")
    fig.tight_layout()
...
if __name__ == "__main__":
    ...
    e = args.epistasis_coefficient

    try:
        fname = f"outputs/Ne_{Ne}_n_{n}_r_{r}_s_{s}_e_{e}.bp"
    ...
    n_proj = 80
    fname = f"expectations/Ne_{Ne}_from_{n_proj}_n_{n}_r_{r}_s_{s}_e_{e}.bp"
    try:
        ...
        rho = 4 * Ne * r
        gamma = - 2 * Ne * s
        sel_params = [2 * gamma * (1 + e), gamma, gamma]
    ...
    plt.savefig(f"plots/comp_Ne_{Ne}_n_{n}_r_{r}_s_{s}_e_{e}.pdf")
    #plt.show()
